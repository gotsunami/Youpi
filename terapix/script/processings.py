...
def print_processings_csv(tasks):
    total = tasks.count()
    k = 1
    for t in tasks[:g_limit]:
        if t.success: state = 'success'
        else: state = 'failure'
        print ';'.join(map(str, [k, t.title[:g_title_width-1], t.start_date, t.end_date-t.start_date, t.user, state, t.results_output_dir]))
        k += 1
    if total > g_limit:
        print "Filtered: latest %d results" % g_limit
    print_footer(total)

def print_processings_std(tasks):
    total = tasks.count()
...
def print_processings(tasks):
    if g_csv:
        print_processings_csv(tasks)
    else:
        print_processings_std(tasks)

def list_processings(tags=[], sort_by='-end_date'):
    """
    List processing results according to parameters
    """
    from terapix.lib.processing import find_tasks
    tasks = find_tasks(tags, g_task_id, g_kind, g_user, g_success, g_failure, sort_by=sort_by)
...
def main():
    ...
    try:
        ...
        kwargs = {}
        if options.tags:
            kwargs['tags'] = options.tags.split(',')
        if options.csv:
            kwargs['sort_by'] = 'title'
        list_processings(**kwargs)
