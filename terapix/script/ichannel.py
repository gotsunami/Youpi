...
if(connectionObject):
	...
	while k <> []:
		...
		test = c.execute("""select Name from spica_channel where name=\'%s\'""" %(chaname));
		if (test == 0):
			
			if (detector == 'MegaCam' and  telescope == 'CFHT 3.6m' and instrname == 'MegaPrime'):
				test1 = c.execute(""" insert into spica_channel set Name =\'%s\',instrument_id = 2 """ %(chaname));
				if (test1):
				...
				test2 = c.execute(""" insert into spica_channel set Name =\'%s\',instrument_id = 3 """ %(chaname));
				if (test2):
					connectionObject.commit();
					print 'transaction done';
				else:
